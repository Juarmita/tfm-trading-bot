...
@pytest.mark.asyncio
@patch.dict(
    "os.environ", {"NEXT_PUBLIC_SUPABASE_URL": "https://mock.supabase.co", "SUPABASE_SERVICE_ROLE_KEY": "mock_key"}
)
...
async def test_get_portfolio_success(mock_get_news, mock_get_quote, mock_get_http):
    ...
    mock_get_quote.side_effect = lambda sym: {
        ...
        "TSLA": Quote(
            symbol="TSLA", price=190.0, change=-2.0, change_pct=-1.04, volume=20000, timestamp=datetime.now()
        ),
    }[sym]
    ...
    mock_get_news.return_value = [
        NewsItem(
            title="AAPL Tech Breakout",
            publisher="Bloomberg",
            link="https://bloomberg.com/aapl",
